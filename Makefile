...
ssh-$(ENV):
	...
	ssh $(SSH_FLAGS) -i $(TMP_SSH_KEY_FILEPATH) \
		ubuntu@$(shell terraform \
		-chdir="./$(ENV_DIR)" output -raw droplet_host)
endef
...
create-repo:
	...
	gh repo create african-cities-journal --public -s . --push -r origin \
		-d "African Cities Journal using PKP OJS"
########################################################################################
